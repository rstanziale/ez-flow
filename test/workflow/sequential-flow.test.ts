...
describe('Sequential flow', () => {
  it('test exectute', async () => {
    ...
    expect(workReport.getWorkContext().isResultSingle()).toBeTruthy();
    expect(workReport.getWorkContext().hasResult()).toBeTruthy();
  ...
  it('test passing with errors', async () => {
    ...
    expect(workReport.getError()).not.toBeUndefined();
    expect(spyWork1).toHaveBeenCalledTimes(1);
    expect(spyWork2).toHaveBeenCalledTimes(1);
    expect(spyWork3).toHaveBeenCalledTimes(0);
